...
export const NewDialog: React.FC = () => {
  ...
  const { setCloseModal, addWidget } = React.useContext(ScreenContext);
  ...
  const renderDialog = React.useMemo(
    ...
    [addWidget, closeModalDialog, selectedValue]
  );
  return renderDialog;
};

const NewModal = () => {
  const { closeModal } = React.useContext(ScreenContext);
  return closeModal ? null : <NewDialog />;
};
